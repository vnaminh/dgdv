...
class Menu
{
    ...
    public static function getDataMenu(){
        ...
        if ($nguoiDungLogin == "" || $nguoiDungLogin == NULL) {
            return;
        }
        ...
        $menu = [];
        $menu_ids = $ql_menu->getMenuIdbyUserId(config::get('app.__APP_NAME'), $nguoiDungLogin);
        if ($menu_ids == "" || $menu_ids == NULL) {
            return $menu;
        }
        // set parent_id cấp cha
        $parent_id = 0;
        $menu_ids = explode(',', $menu_ids);
        $menu_list0 = $ql_menu->getMenuRecursion(config::get('app.__APP_NAME'), $menu_ids, $parent_id);



        if (!empty($menu_list0) && $menu_ids != null) {
            $issetLV2 = false;
            foreach ($menu_list0 as $item0) {
                $parent_id0 = $item0->ql_menu_id;
                $menu_name0 = $item0->ql_menu_name;
                $controller0 = $item0->ql_menu_controller;
                $icon0 = $item0->ql_menu_icon;

                //Cắt chuổi để lấy route
                $menu_list1 = $ql_menu->getMenuRecursion(config::get('app.__APP_NAME'), $menu_ids, $parent_id0);
                foreach ($menu_list1 as $key => $value) {
                    $menuTMP = $ql_menu->getMenuRecursion(config::get('app.__APP_NAME'), $menu_ids, $value->ql_menu_id);
                    if(empty($menuTMP)) {
                        unset($menu_list1[$key]);
                    }
                }
                $menu_listTMP = [];
                foreach ($menu_list1 as $value){
                    if($value->ql_menu_controller == '#'){
                        $dataTMP = $ql_menu->getMenuRecursion(config::get('app.__APP_NAME'), $menu_ids, $value->ql_menu_id);
                        foreach ($dataTMP as $k => $v){
                            $menu_listTMP[$v->ql_menu_controller][] = $v;
                        }
                    }
                    $menu_listTMP[$value->ql_menu_controller][] = $value;
                }

                if (!empty($menu_list1) && count($menu_list1)> 0) {
                    $dataTMPCheck = [];
                    foreach ($menu_list1 as $k => $v) {
                        if($v->ql_menu_controller == "#"){
                            $issetLV2 = true;
                        }else{
                            $issetLV2 = false;
                        }
                        $dataTMPCheck[] = $v->ql_menu_id;
                    }
                    $checkIssetMenuLv2 = $ql_menu->getMenuRecursion(config::get('app.__APP_NAME'), $menu_ids, $dataTMPCheck);
                    if(($checkIssetMenuLv2->isNotEmpty() || !$issetLV2) ){
                        $menu['items'][$parent_id0]['title'] = $menu_name0;
                        $menu['items'][$parent_id0]['icon'] = $icon0;
                        $menu['items'][$parent_id0]['bullet'] = 'dot';
                        $menu['items'][$parent_id0]['root'] = true;
                    }
                    foreach ($menu_list1 as $item1) {
                        $parent_id1 = $item1->ql_menu_id;
                        $menu_name1 = $item1->ql_menu_name;
                        $controller1 = $item1->ql_menu_controller;
                        $menu['items'][$parent_id0]['submenu'][$parent_id1]['title'] = $menu_name1;
                        $menu['items'][$parent_id0]['submenu'][$parent_id1]['page']= $controller1;
                    }
                } else if ($controller0 != "#") {
                    $menu['items'][$parent_id0]['title'] = $menu_name0;
                    $menu['items'][$parent_id0]['icon'] = $icon0;
